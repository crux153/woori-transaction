...
module.exports = async function(account, password, birthday, range = "1W") {
	try {
		...
		const error = await page.evaluate(() => {
			const $error = document.querySelector("#error-area-TopLayer .error-area .mb10");
			if (!$error) {
				return null;
			} else {
		...
		await page.evaluate(range => {
			window.setToday();
			window.setCalTerm(true, "INQ_END_DT", "INQ_STA_DT", range.unit, range.amount);
			window.doSubmit();
		}, range);
		...
		for (let currentPage = 1; currentPage <= totalPages; currentPage++) {
			const result = await page.evaluate(() => {
				...
				const replaceFullWidth = str => str.replace(/[\uFF01-\uFF5E]/g, ch => String.fromCharCode(ch.charCodeAt(0) - 0xFEE0));
